<template>
  ...
  <div style="height: 670px;">
    ...
    >
      ...
      <!-- <control-marker :map="map" position="bottomright" @perspective="perspective"></control-marker> -->
...
export default {
  ...
  mounted () {
    this.map = this.$refs.map1.mapObject
  ...
  methods: {
    ...
    mouseEnterLeft () {
      console.log('执行')
      this.$refs.map1.mapObject.on('@update:zoom', this.zoomUpdated)
      this.$refs.map1.mapObject.on('@update:center', this.centerUpdated)
    },
    mouseLeaveLeft () {
      this.$refs.map1.mapObject.off('update:zoom', this.zoomUpdated)
    },
    mouseEnterRight () {
      console.log('移入右')
    },
