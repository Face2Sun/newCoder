<template>
  <div>
    ...
    <a-card style="margin:30px auto">
      <div>
        <a-row type="flex">
          <a-col type="flex" :span="6">
            <span><h1>事项列表</h1><p>共{{ counter }}个事项</p></span>
          </a-col>
          <a-col :span="18">
      ...
      >
        <span slot="bssx" slot-scope="text,record">
          ...
          <span style="float: right">
            <a-button id="blBtn1" type="primary" @click="showModel(record.bsznid)">在线办理</a-button>
            <a-button id="blBtn2" type="primary" @click="showModel(record.bsznid)">办事指南</a-button>
          </span>
        </span>
      </a-table>
      <guide-maintain ref="guideMaintain"></guide-maintain>
    </a-card>
  </div>
</template>
<script>
import { findXzsxBsznListByBsznCode, findXzsxBsznListLikeBssxAndCode } from '@/api/businessGuide'
import GuideMaintain from './maintain/GuideMaintain'
...
export default {
  ...
  methods: {
    // 弹出模块框
    showModel (record) {
      this.$refs.guideMaintain.showModel()
      this.$refs.guideMaintain.loadData(record)
    },
    loadTable (index) {
