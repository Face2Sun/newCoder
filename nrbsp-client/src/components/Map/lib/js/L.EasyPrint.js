...
L.Control.EasyPrint = L.Control.extend({
  ...
  printMap: function (event, filename) {
    ...
    if(this.originalState.mapHeight.includes('%')){
  ...
  _createImagePlaceholder: function (sizeMode) {
    ...
    domtoimage.toPng(this.mapContainer, {
      ...
      .catch(function (error) {
          ...
          //Edit
          var mapContainer=plugin._map.getContainer();
          var mapContainerParent=mapContainer.parentNode;
          if(!mapContainerParent.hasAttribute("id")){
            var mapMoudelWrapper=mapContainerParent.parentNode;
            mapMoudelWrapper.appendChild(mapContainer);
            mapMoudelWrapper.removeChild(mapContainerParent);
          }
          plugin._toggleControls(true);
          loft.toast('error','地图导出错误，请检查网络环境！');
      });
  },
  ...
  _createOuterContainer: function (mapDiv) {
    ...
    // var mapInfoWidth=$(mapDiv.parentNode.childNodes[0]).width();
    // if(mapInfoWidth>50){
    //   outerContainer.style.marginLeft=mapInfoWidth+"px";
  ...
  _toggleControls: function (show) {
    ...
    var controlContainer = mapContainer.getElementsByClassName("leaflet-control-container")[0];
    var mapInfoContainer=mapContainer.getElementsByClassName("leaflet-mapInfo-display")[0];
    if (show) {
