...
L.Control.Measure = L.Control.extend({
	...
	addDashLine:function(e){
		...
		if(this.poly_points.length>0){
			...
			this.dashLine.setLatLngs(dash_points);
	...
	addDeleteBtn:function(lastIcon){
		...
		var deleteImg =  L.DomUtil.create('img', 'leaflet-measure-deleteBtn',divIconArr[divIconArr.length-1].childNodes[0]);
		deleteImg.type = 'img';
		deleteImg.src = "/loft-desktop/src/heavy-third-party/webgis/js/images/clearGray.png";
	...
	showPoly:function(e){
		...
		for(i=0;i<this.poly_points.length;i++){
	...
	clearMeasure:function(e){
		this.cancleMeasureListener();
		for(i=0;i<this.measure_layerGroup.length;i++){
	...
	cancleMeasureListener: function(){
		...
		if(this._measureContainer){
			for(var i=0;i<this._measureContainer.childNodes.length;i++){
