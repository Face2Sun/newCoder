...
L.Control.PixelFilter = L.Control.extend({
  ...
  initialize: function (url, options) {
    options = L.extend({}, L.Control.prototype.options, {
      matchRGBA: null,
      missRGBA: null,
      pixelCodes: [],
      crossOrigin: 'Anonymous' // per issue 15, this is how you do it in Leaflet 1.x
    }, options)
    L.Control.prototype.initialize.call(this, url, options)
  ...
  setMatchRGBA: function (rgba) {
    // save the setting
    if (rgba !== null && (typeof rgba !== 'object' || typeof rgba.length !== 'number' || rgba.length !== 4)) alert('L.Control.PixelSwap expected matchRGBA to be RGBA [r,g,b,a] array or else null')
  ...
  setMissRGBA: function (rgba) {
    // save the setting
    if (rgba !== null && (typeof rgba !== 'object' || typeof rgba.length !== 'number' || rgba.length !== 4)) alert('L.Control.PixelSwap expected missRGBA to be RGBA [r,g,b,a] array or else null')
  ...
  setPixelCodes: function (pixelcodes) {
    // save the setting
    if (typeof pixelcodes !== 'object' || typeof pixelcodes.length !== 'number') alert('L.Control.PixelSwap expected pixelCodes to be a list of triplets: [ [r,g,b], [r,g,b], ... ]')
  ...
  // this is really only for Leaflet 0.7; as of 1.0 L.Control has a crossOrigin setting which we define as a layer option
  _createTile: function () {
    var tile = L.Control.prototype._createTile.call(this)
  ...
  applyFiltersToTile: function (imgelement) {
    ...
    var matchRGBA = this.options.matchRGBA; var missRGBA = this.options.missRGBA
    if (matchRGBA !== null) {
      var matchR = matchRGBA[0]; var matchG = matchRGBA[1]; var matchB = matchRGBA[2]; var matchA = matchRGBA[3]
    }
    if (missRGBA !== null) {
      var missR = missRGBA[0]; var missG = missRGBA[1]; var missB = missRGBA[2]; var missA = missRGBA[3]
    }
    ...
    try {
      ...
      alert('L.Control.PixelFilter getImageData() failed. Likely a cross-domain issue?')
    }
    for (let i = 0, n = pixels.length; i < n; i += 4) {
      ...
      if (pixelcodes.length) {
        ...
        if (pixelcodes.indexOf(sum) === -1) match = false
      }
      ...
      output.data[i] = match ? (matchRGBA === null ? r : matchR) : (missRGBA === null ? r : missR)
      output.data[i + 1] = match ? (matchRGBA === null ? g : matchG) : (missRGBA === null ? g : missG)
      output.data[i + 2] = match ? (matchRGBA === null ? b : matchB) : (missRGBA === null ? b : missB)
      output.data[i + 3] = match ? (matchRGBA === null ? a : matchA) : (missRGBA === null ? a : missA)
    }
...
L.control.PixelFilter = function (options) {
  return new L.Control.PixelFilter(options)
}
